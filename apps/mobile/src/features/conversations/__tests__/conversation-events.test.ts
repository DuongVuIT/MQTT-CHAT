...
const conv = (id: string, over: Partial<ApiConversation> = {}): ApiConversation => ({
  id,
  type: "GROUP",
  title: id,
  memberCount: 1,
...
describe("applyConversationEvent — Web→Mobile discovery", () => {
  ...
  it("duplicate created events collapse to ONE entity (QoS1 / REST+event race)", () => {
    ...
    list = applyConversationEvent(list, "conversation.created", createdEvent("g1").data, "user-me");
    list = applyConversationEvent(list, "conversation.created", createdEvent("g1").data, "user-me");
...
describe("applyReactionEvent — cross-client reactions in realtime", () => {
  const msg = (id: string, reactions: ApiMessage["reactions"] = []): ApiMessage => ({
    id,
  ...
  it("reaction.added from ANOTHER client renders immediately (parity with Web)", () => {
    const next = applyReactionEvent([msg("m1")], "reaction.added", reactionData);
...
describe("applyReadReceipt — canonical read state", () => {
  ...
  it("advances MY watermark → my unread badge (lastSequence − myRead) drops", () => {
    ...
    const mine = next.find((c) => c.id === "c1")!.members.find((m) => m.userId === "user-me")!;
  ...
  it("advances a PEER's watermark → their ✓✓ ticks move live", () => {
    ...
    const peer = next.find((c) => c.id === "c1")!.members.find((m) => m.userId === "user-peer")!;
  ...
  it("NEVER regresses on out-of-order receipts", () => {
    ...
    expect(
      regressed.find((c) => c.id === "c1")!.members.find((m) => m.userId === "user-me")!
        .lastReadSequence,
    ).toBe(9);
  });
